...
class ProcessManager {
  ...
  async keepRunning({
    ...
    setInterval(async () => {
      ...
      if (prInfo.PID) {
        ...
        this.notRespondingCnt += 1;
      } else {
        this.notRespondingCnt = 0;
      }

      // if the process does not responding to long then kill this process
      if (prInfo.PID && this.notRespondingCnt === notRespondingCheckLimit) {
        this.notRespondingCnt = 0;
  ...
  /**
   * Resets not responding count if AE-manager detects interaction
   * 
   * @returns {void}
   * @author Madis
   * @since 2020.05.19
   */
  async notifyInteractive() {
    console.log(`[${getDate()}] (PManager) interaction notification received`);
    this.notRespondingCnt = 0;
  }
  ...
  async getProcessInfo({
    ...
    return new Promise(((resolve) => {
      ...
      exec(`tasklist /FO CSV /V /FI ${cmdFilter}`, (_err, stdout) => {
        ...
        const h = prList.shift();

        if (h.length === 0) {
          return resolve(result);
        }

        const header = h.split(',').map(v => v.replace(/"| |\r/g, ''));
        ...
        const rowValues = rowVal.split(',').map(v => v.replace(/"|\r/g, ''));
        // eslint-disable-next-line no-return-assign
